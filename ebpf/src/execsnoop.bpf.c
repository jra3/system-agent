...
int tracepoint__syscalls__sys_enter_execve(struct trace_event_raw_sys_enter *ctx)
{
    ...
    // First, let's capture the filename as the first argument
    if (filename) {
        int ret = bpf_probe_read_user_str(&event->args[0], ARGSIZE, filename);
        if (ret > 0) {
            event->base.args_count = 1;
    ...
    // Save event to hash map for sys_exit_execve

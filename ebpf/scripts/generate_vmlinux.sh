...
BTFHUB_URL="https://github.com/aquasecurity/btfhub-archive/raw/main/ubuntu/20.04/${BTFHUB_ARCH}/5.4.0-91-generic.btf"
...
if ! curl -sL "${BTFHUB_URL}" -o "${TMP_DIR}/vmlinux.btf"; then
...
if command -v bpftool &> /dev/null; then
    ...
    bpftool btf dump file "${TMP_DIR}/vmlinux.btf" format c > "${VMLINUX_H}"
